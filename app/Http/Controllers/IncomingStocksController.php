...
class IncomingStocksController extends Controller
{
    ...
    public function getAvailableIncomingStocks($productId = null)
    {
        // Validate the product ID
        if (!$productId || !Product::where('id', $productId)->where('is_machine', true)->exists()) {
            return response()->json(['error' => 'Invalid or non-machine product ID'], 404);
        }
    
        // Get all incoming stock IDs related to the product where `is_machine = true`
        $incomingStockIds = IncomingStock::where('product_id', $productId)->pluck('id');
    
        // Find incoming stocks that are NOT in the outgoing stock table and include supplier data
        $availableIncomingStocks = IncomingStock::whereIn('id', $incomingStockIds)
            ->whereNotIn('id', OutgoingStock::pluck('incoming_stock_id')) // Exclude used incoming stocks
            ->with('product.supplier') // Include supplier details
            ->get();
    
        return response()->json($availableIncomingStocks);
    }
}